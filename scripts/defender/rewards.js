...
const monarch = '0x69fa0fee221ad11012bab0fdb45d444d3d2ce71c';
...
exports.handler = async function (event) {
  ...
  const pool = pools.find((p) => p.asset === `ETH.MONARCH-${monarch}`.toUpperCase());
  ...
  console.log(`rewards: ${rewards} MONARCH (${(rewards*price).toFixed(0)} USD) (price: ${price.toFixed(3)})`);
  ...
  const tx = await contract.deposit(
    ...
    `DONATE:ETH.MONARCH-${monarch}`.toUpperCase(),
    { gasLimit: 1000000 }
  );
  // await tx.wait();
  return { hash: tx.hash };
